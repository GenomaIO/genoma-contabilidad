...
export default function BalanceComprobacion() {
    ...
    async function fetchBalance() {
        ...
        try {
            const res = await fetch(`${apiUrl}/ledger/trial-balance?period=${period}`, {
        ...
        <div style={{ padding: '24px', maxWidth: 960, margin: '0 auto', fontFamily: 'Inter, sans-serif' }}>
            ...
            {!loading && data && (
                <div style={{ border: '1px solid var(--border-color)', borderRadius: 10, overflow: 'hidden' }}>
                    {/* Cabecera */}
                    <div style={{ display: 'grid', gridTemplateColumns: '90px 1fr 80px 120px 120px 110px', gap: 8, padding: '10px 16px', background: 'rgba(124,58,237,0.1)', fontSize: '0.75rem', fontWeight: 700, color: 'var(--text-muted)' }}>
                        <span>CÓDIGO</span>
                        <span>NOMBRE</span>
                        <span>TIPO</span>
                    ...
                    {accounts.map((acc, i) => {
                        ...
                        const saldo = (acc.total_debit || 0) - (acc.total_credit || 0)
                        return (
                            <div
                                key={acc.account_code}
                                id={`balance-row-${acc.account_code}`}
                                style={{ display: 'grid', gridTemplateColumns: '90px 1fr 80px 120px 120px 110px', gap: 8, padding: '9px 16px', borderTop: '1px solid var(--border-color)', fontSize: '0.82rem', background: i % 2 === 0 ? 'transparent' : 'rgba(0,0,0,0.03)' }}
                            >
                                <span style={{ fontFamily: 'monospace', color, fontWeight: 700 }}>{acc.account_code}</span>
                                <span style={{ color: 'var(--text-primary)' }}>{acc.account_name || acc.account_code}</span>
                                <span style={{ fontSize: '0.72rem', color }}>{acc.account_type}</span>
                                <span style={{ textAlign: 'right', color: '#3b82f6' }}>
                                    {acc.total_debit > 0 ? `¢${Number(acc.total_debit).toLocaleString('es-CR', { minimumFractionDigits: 2 })}` : '—'}
                                </span>
                                <span style={{ textAlign: 'right', color: '#10b981' }}>
                                    {acc.total_credit > 0 ? `¢${Number(acc.total_credit).toLocaleString('es-CR', { minimumFractionDigits: 2 })}` : '—'}
                                </span>
                                <span style={{ textAlign: 'right', color: saldo >= 0 ? '#10b981' : '#ef4444', fontWeight: 600 }}>
                                    ¢{Math.abs(saldo).toLocaleString('es-CR', { minimumFractionDigits: 2 })}
                                    {saldo < 0 && <span style={{ fontSize: '0.7rem', marginLeft: 2 }}>CR</span>}
                                </span>
                            </div>
                        )
                    })}
                    ...
                    {/* Totales */}
                    <div style={{ display: 'grid', gridTemplateColumns: '90px 1fr 80px 120px 120px 110px', gap: 8, padding: '12px 16px', borderTop: '2px solid var(--border-color)', fontWeight: 700, fontSize: '0.85rem', background: 'rgba(0,0,0,0.05)' }}>
                        <span></span>
